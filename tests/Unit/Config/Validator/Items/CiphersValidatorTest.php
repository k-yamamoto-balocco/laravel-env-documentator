...
/**
 * @coversDefaultClass \GitBalocco\LaravelEnvDocumentator\Config\Validator\Items\CiphersValidator
 ...
 */
class CiphersValidatorTest extends TestCase
{
    static public function validateDataProvider(): array
    {
        return [
            ...
            "CipherValidatorをMockしているため、trueと判断される" => [
                ['dest101' => 'encrypted_file_path101'],
                ['dest101'],
                true
            ],
            "実際に利用する場合は、cipherとして取りうる値のみ許可される" => [
                [
                    'dest201' => 'aes-128-cbc',
                    'dest202' => 'aes-256-cbc',
                    'dest203' => 'aes-128-gcm',
                ...
                ['dest201', 'dest202', 'dest203', 'dest204'],
                true
            ],
            "0はNG" => [
                0,
    ...
    {
        $mock = \Mockery::mock(CipherValidator::class);
        //
        $mock->allows('definition')->andReturn(Validator::alwaysValid());
        $class = new CiphersValidator($mock, $destinations, $candidate);
